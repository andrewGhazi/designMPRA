...
sim_power = data_frame(transcription_shift = seq(-1.5, 1.5, length.out = 200),
                       theoretical_power = pwr.t.test(n = nrow(worst),
                                                      ...
                                                      sig.level = .001)$power,
                       monte_carlo_power = mclapply(transcription_shift, simulate_power, mc.cores = 22, sig_level = .001, n_sim = 30000) %>% unlist)

```

```{r}
load('~/designMPRA/outputs/sim_power.RData')
...
The two curves are almost exactly the same. 
