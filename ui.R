...
shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(conditionalPanel(condition = 'input.selectedTab==3',
      ...
      tabsetPanel(
        ...
        tabPanel("Power", 
                 ...
                 selectInput('parameter',
                             label = h4('Choose parameter to investigate'),
                             choices = list('n' = 1, 
                                            'Power' = 2,
                                            'Difference in mean activity' = 3,
                                            'significance level' = 4),
                             selected = 3),
                 numericInput('nbarcode',
                              label = h4('n barcodes per snp'),
                              value = 20),
                 p('This plot shows the power of a t-test to distinguish differential activity in a variant at a Bonferonni corrected alpha = .05 level. While activity measurements do not strictly satisfy the assumptions of a t-test (they are not normally distributed), in practice a normal approximation works well.'),
                 br(),
                 plotOutput('powerPlot'),
                 value = 2),
        tabPanel("Create sequences",
                 p('Here you can upload a VCF to automatically generate the necessary construct sequences for your own MPRA experiment. Only the CHROM, POS, REF, and ALT columns are used. Current constraints are: '),
                 tags$div(
                   tags$ul(
                     tags$li('sequence context comes from the hg38 assembly'),
                     tags$li('Restriction sites are limited to XbaI, KpnI, and SfiI as they are used in MPRA protocol published in Melnikov (2014).'),
